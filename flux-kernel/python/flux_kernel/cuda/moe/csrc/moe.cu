...
// Directly adapted from SGL Kernel 0.3.15 (8c9670375).
TORCH_LIBRARY_FRAGMENT(flux_kernel, m) {
  m.def("moe_fused_gate(Tensor input, Tensor bias, int groups, int selected_groups, int topk, int fused_shared, float scale, bool apply_scale) -> Tensor[]", &moe_fused_gate);
  m.def("moe_align_block_size(Tensor topk_ids, int experts, int block_size, Tensor(a!) sorted_ids, Tensor(b!) expert_ids, Tensor(c!) total, Tensor(d!) cumsum, bool fill_padding) -> ()", &moe_align_block_size);
}
