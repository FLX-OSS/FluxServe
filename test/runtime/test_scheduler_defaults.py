...
def test_scheduler_policy_defaults_to_paged():
    assert ServerArgs().scheduler_policy == "paged"
...
@pytest.mark.parametrize("policy", ["paged", "dynamic"])
...
def test_cli_rejects_removed_default_scheduler_policy():
    with pytest.raises(SystemExit):
        build_parser().parse_args(["serve", "--model", "model", "--scheduler-policy", "default"])
