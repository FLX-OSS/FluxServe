...
from ci_system import cuda_coredump

def kill_process_tree(pid: int) -> None:
    """Terminate a test process and its descendants using its process group."""
    try:
...
def run_unittest_files(
    files: List[Union[TestFile, str, object]],
    ...
    for i, file in enumerate(files):
        if hasattr(file, "filename") and hasattr(file, "est_time"):
            filename, estimated_time = file.filename, file.est_time
        elif isinstance(file, str):
            filename, estimated_time = file, 60
        else:
