...
The [HTTP entrypoint](../python/fluxserve/backend/entrypoints/http_server.py) provides health checks and completion endpoints, including `/v1/completions` and `/v1/chat/completions`. The [CLI](../python/fluxserve/cli/launch.py) configures model loading, scheduler policy, cache layout, decoding, and distributed execution.
